...
class ExplanationBaseForTransformer(ExplanationBase):
    ...
    def explain_sst(self):
        ...
        file_list=[]
        for i in range(12):
            f = open(self.output_path+'.'+str(i), 'w')
            file_list.append(f)
        ...
        for batch_idx, (input_ids, input_mask, segment_ids, label_ids, offsets, mappings) in enumerate(self.iterator):
            ...
            contribs_ = np.array(contribs)
            for i in range(contribs_.shape[1]):
                s = self.repr_result_region(inp, repr_spans, contribs_[:,i])
                file_list[i].write(s + '\n')
...
class SOCForTransformer(ExplanationBaseForTransformer):
    ...
    def score(self, inp_bak, inp_mask, segment_ids, x_regions, nb_regions, label):
        ...
        logits_enb_list = self.model(inp_enb, segment_ids[:, :inp_enb.size(1)], inp_enb_mask)
        logits_ex_list = self.model(inp_ex, segment_ids[:, :inp_ex.size(1)], inp_ex_mask)
        contrib_list = []
        for (i,j) in zip(logits_enb_list, logits_ex_list):
            contrib_logits = i - j  # [B * 2]
            if contrib_logits.size(1) == 2:
                contrib_score = contrib_logits[:, 1] - contrib_logits[:, 0]  # [B]
            else:
                contrib_score = normalize_logit(contrib_logits, label)
            contrib_score = contrib_score.mean()
            contrib_list.append(contrib_score.item())
        return contrib_list
